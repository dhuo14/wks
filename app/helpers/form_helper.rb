...
module FormHelper
    # 树形选择框
    def tree_select(name, rs,  url, options = {})
      ...
      # (text_field_tag("box_#{name}", name_value, options.merge({:id => "box_#{input_id}"})) + hidden_field_tag(name, id_value, :id => input_id, :droptree_id => "box_#{input_id}" )).html_safe

      str = %Q|
      <div class="input-group">
      #{text_field_tag("box_#{name}", name_value, {:id => "box_#{input_id}", :class => "form-control", :readonly => "readonly", :placeholder => "请选择..."})}
      #{hidden_field_tag(name, id_value, {:id => input_id, :class => "tree_select", :url => url, :checked_max => checked_max, :keyword => keyword})}
      ...
      #{tree_select_modal(input_id, url, checked_max, keyword)}
      |
      str.html_safe
    end
    ...
    def tree_select_modal(id,url,checked_max,keyword)
      %Q{
